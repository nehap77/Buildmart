...
public interface VendorProduct_Repo extends JpaRepository<Vendor_Product, Integer> {
	...
	@Modifying
	@Query(value="update vendor_product e set e.quantity=:quantity, e.price =:price, e.offer_percentage=:offerPercentage, e.offer_valid_date =:offerValidDate  WHERE e.id = :vid",nativeQuery = true)
